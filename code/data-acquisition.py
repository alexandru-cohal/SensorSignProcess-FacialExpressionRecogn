...
def main():
    ...
    path_to_save = "rawdata_01-03-18/"

    n_samples_to_record = 100
    data_array = np.zeros((n_samples_to_record,14))

    time_to_react = 1

    print "Prepare to act in :"
    print "3"
    time.sleep(1)
    print "2"
    time.sleep(1)
    ...
    print "ACTION!"
    index_sample = 0 
    while index_sample < n_samples_to_record:
        
        try:
            data = e.get_sample()
            if data:   
                if (data[0] != 0):            
                    data_array[index_sample,:] = data                    
                    index_sample = index_sample + 1
            
        except EPOCTurnedOffError, ete:
            print "ERRORRRRRRRRRR...WOWWWW!"
        except KeyboardInterrupt, ki:
            e.disconnect()
            return 0
		
    np.save(path_to_save + file_name, data_array)
