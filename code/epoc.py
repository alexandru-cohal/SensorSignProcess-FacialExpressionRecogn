...
def main():
    ...
    n_instance = sys.argv[2]

    print "Time to run = " + str(time_to_run)
    print e.sampling_rate

    n_samples = int(time_to_run * e.sampling_rate)
    data_array = np.empty([n_samples, 14])
    quality_array = np.empty([n_samples, 14])

    for label in range(1):
        print "Start of recording " + str(label)
        for index_sample in range(n_samples):
            try:
                ...
                if data:
                    ...
                    for index_channel,channel in enumerate(e.channel_mask):
                        #print "%10s: %.2f %20s: %.2f" % (channel, data[index_channel], "Quality", e.quality[channel])
                        data_array[index_sample, index_channel] = data[index_channel]
                        quality_array[index_sample, index_channel] = e.quality[channel]
                ...
                e.disconnect()
                return 0
        print "End of recording " + str(label)
        np.save("data" + "_" + n_instance +".npy", data_array)
        time.sleep(2)
        
        plt.figure()
        for i in range(14):   
            plt.subplot(7,2,i+1)
            plt.plot(range(0,n_samples), data_array[:,i])
        plt.show()    
