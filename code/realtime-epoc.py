...
def main():
	...
	fig.canvas.draw()
	n_samples_plot = 10    
	realtime_data = np.zeros((n_samples_plot,14))
		...
		try:
			...
			if data:
				...
				for channel in range(3,4):
					realtime_data[0:n_samples_plot-1, channel] = realtime_data[1:n_samples_plot, channel]
					realtime_data[-1, channel] = data[channel]
					ax = plt.subplot(7,2,channel+1)
					plt.cla()
					ax.plot(realtime_data[:,channel])
					plt.ylim([1000, 4000])
					ax.grid()
				print time.time() - t
				fig.canvas.draw() 
